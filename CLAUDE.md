...
Plus **Shaders surface** — Bleepforge's tenth surface, file-based like Assets: the `.gdshader` text files in the Godot project ARE the source of truth, no JSON cache, no `.tres` round-trip. Seven phases shipped — browse, view, edit, save, new (from template), duplicate, delete, cross-system usage tracking, CodeMirror editor with GDShader syntax + gutter diagnostic markers, watcher + SSE for live cross-window sync, a live WebGL2 preview canvas that re-translates GDShader → GLSL ES on every edit with auto-generated uniform controls (sliders for `hint_range`, color pickers for `source_color`), multi-texture support (user-declared `sampler2D` uniforms get per-uniform AssetPickers + dedicated WebGL texture units), helper-function support (top-level user functions ride into the emit with built-in references auto-substituted), AND sampler-hint semantics — `filter_*` / `repeat_*` drive runtime `texParameteri`, `hint_screen_texture` aliases the sampler to the picked test image so Godot post-process shaders preview against a representative screenshot. Covered in the "Shaders surface" section below.
