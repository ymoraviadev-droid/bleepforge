...
- **Tint section** — three dials, all on the [SliderField](client/src/components/SliderField.tsx) + [PixelSlider](client/src/components/PixelSlider.tsx) chrome:
  ...
  - **Bg** (0-100%) — extends the tint *color* into transparent pixels at this strength. The "tint reaches the bg too" shortcut: subject + bg in the same tint color in one move, without opening the Bg color section. 0 = visible-only (default).
- **Bg color section** — independent backdrop with its own color picker + Alpha. Fills only fully-transparent pixels (alpha === 0). Use this when the bg color should differ from the tint color. Tint > Bg and Bg color are complementary, not redundant: Tint > Bg uses the *tint color* (one-color-wash flow); Bg color uses an independent color (two-different-colors flow). They're mostly mutually exclusive at the per-pixel level — bg color only touches pixels still at alpha === 0 after tint runs, so if Tint > Bg already painted them, bg color won't override.
- **Pipeline at save**: `working → tint (incl. its bgFill) → bg color → crop → PNG`.
