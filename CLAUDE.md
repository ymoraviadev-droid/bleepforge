...
Plus **Shaders surface** — Bleepforge's tenth surface, file-based like Assets: the `.gdshader` text files in the Godot project ARE the source of truth, no JSON cache, no `.tres` round-trip. Six phases shipped — browse, view, edit, save, new (from template), duplicate, delete, cross-system usage tracking, CodeMirror editor with GDShader syntax + gutter diagnostic markers (parser + WebGL errors line-anchored), watcher + SSE for live cross-window sync, a live WebGL2 preview canvas that re-translates GDShader → GLSL ES on every edit with auto-generated uniform controls (sliders for `hint_range`, color pickers for `source_color`), multi-texture support (user-declared `sampler2D` uniforms get per-uniform AssetPickers + dedicated WebGL texture units), AND helper-function support — top-level user functions other than `fragment()` ride into the emitted GLSL with built-in references auto-substituted to their underlying uniform / varying form. Covered in the "Shaders surface" section below.
...
- **Hints**: `hint_range(min, max[, step])`, `source_color` (aliased `hint_color`), and the sampler hints `filter_nearest` / `filter_linear` / `filter_nearest_mipmap` / `filter_linear_mipmap` / `repeat_enable` / `repeat_disable`. The first two drive UI control generation (slider / color picker); the sampler hints are accepted as no-ops at the runtime level (preview always samples NEAREST + CLAMP for pixel-art fidelity) but parsing them means shaders copied from Godot tutorials don't need manual editing to compile.
