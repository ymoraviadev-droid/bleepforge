...
- **Click affordances baked into [AssetThumb](client/src/components/AssetThumb.tsx).** Default behavior: when `path` is set, the thumb is clickable — click → open editor in `edit` mode for that path, right-click → Edit / Duplicate / Delete menu. Both handlers `stopPropagation` so the thumb's behavior doesn't fight a surrounding clickable parent (e.g. an NPC card linking to `/npcs/:id` — click on the card body navigates, click on the portrait edits the portrait). One opt-out: `editable={false}` on the AssetPicker browse modal's file rows, where the row IS a `<button>` whose click means "pick this image for the field". Everywhere else (NPC portrait, item icon, dialog line portrait, faction icon + banner, balloon speaker portrait, concept hero, gallery card thumb, AssetPicker text-field preview) inherits the click-to-edit behavior automatically. Right-click → Edit / Duplicate / Delete is universal — the picker browse rows still get it via `<li onContextMenu>` even though their thumbs are inert.
- The gallery's `AssetCard` / `AssetRow` (the `/assets` page) keep their own whole-card click handler too (above and beyond the inner thumb's click) since clicking ANYWHERE on a gallery card should open the editor — they're `role="button"` + `tabIndex` + Enter/Space keyboard handlers + `cursor: pointer` for accessibility. The "used by N" pill `stopPropagation`s so clicking the pill opens the usages drawer rather than the editor.
