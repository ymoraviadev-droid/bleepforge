...
Plus **Shaders surface** — Bleepforge's tenth surface, file-based like Assets: the `.gdshader` text files in the Godot project ARE the source of truth, no JSON cache, no `.tres` round-trip. Eight phases shipped — browse, view, edit, save, new (from template), duplicate, delete, cross-system usage tracking, CodeMirror editor with GDShader syntax + gutter diagnostic markers, watcher + SSE for live cross-window sync, a live WebGL2 preview canvas that re-translates GDShader → GLSL ES on every edit with auto-generated uniform controls (sliders for `hint_range`, color pickers for `source_color`), multi-texture support (user-declared `sampler2D` uniforms get per-uniform AssetPickers + dedicated WebGL texture units), helper-function support (top-level user functions ride into the emit with built-in references auto-substituted), sampler-hint semantics (`filter_*` / `repeat_*` drive `texParameteri`, `hint_screen_texture` aliases to the picked test image), AND ping-pong framebuffers for iterative effects — `hint_previous_frame` on a sampler reads from a back-FBO that holds the last frame's rendered output, unlocking trails / decay / feedback shaders. Covered in the "Shaders surface" section below.
...
- **Splash screen** ([client/src/components/SplashScreen.tsx](client/src/components/SplashScreen.tsx)) fires on every fresh mount of the app (i.e. real refresh / first load) — except in popout windows, which skip it (`showSplash = !popout` initial state — popouts are focused subviews, not full sessions). A centered content block (sized at `min(80vw, 520px)`) on a full-window dark overlay (`fixed inset-0`); the overlay hides the app shell hydrating underneath so the user never sees a partially-painted UI flicker through. The splash content has no border of its own — the OS window frame is the only edge (the splash matches the OS window at 640×520; see "Window mode + menu" below). Inside: BLEEPFORGE display title, pixel progress bar, percent label, and a slot below for either the flavor status line or the CONTINUE button. The current URL is preserved across the splash because the router doesn't re-mount — F5 on `/quests` goes splash → continue → `/quests`. The same `<CreditLine>` rendered by the footer is pinned to the overlay's bottom (not the card) so the authorship attribution greets you on both ends.
...
**Navbar placement**: between Shaders and Assets — `Game concept | ... | Items | Shaders | Game codex | Assets`. Codex + Assets are the trailing "catch-all" pair: Codex for design content without a schema, Assets for files; Shaders sits just before them as the last domain-specific surface. `Game concept` stays leftmost as the homepage; the linguistic pair (`Game concept` + `Game codex`) is acknowledged but not visually clustered, since their shapes (single doc vs multi-category authoring) are different enough that users won't expect them to behave the same.
...
**Initial seed**: 10 categories, 65 entries. Getting started (red, 5), Editor basics (emerald, 8), Game domains (blue, 6), Dialogs and the graph (cyan, 9), Balloons (pink, 3), Game codex (violet, 4), Assets and image editor (orange, 7), Diagnostics (amber, 7), Under the hood (pink, 8), Shaders (lime, 8 — the eighth being `iterative-effects` covering Phase 7b's `hint_previous_frame` + trails). Authored at the level of "what would surprise a new contributor," with cross-links between related entries. The Shaders category landed alongside Phase 4 of the shader work and was the trigger for adding `lime` to the shared palette helper — the 9th color slot, used by the shader category's accent stripe + the AppSearch kind badge + the canvas_item card tint.
...
**No self-write suppression on cache / SSE.** The watcher upserts the cache + publishes SSE on every `.gdshader` change regardless of who wrote it — keeps other open windows in sync. The saving window's own edit page would see its save echoed back, but the edit page's `useShaderRefresh` callback checks `dirty === false` and does a silent no-op reload in that case (re-fetched content matches what was just saved, so the doc identity doesn't shift). The earlier draft did suppress self-writes for the entire pipeline; smoke testing showed it also skipped the cache update + SSE publish, leaving other windows stale, so it was rolled back to the uniform-flow design.

**Self-write IS suppressed for two downstream consumers though** — both are layered on top of the always-firing SSE without changing it:

- **Saves activity feed** (server-side, [server/src/lib/shaders/selfWrite.ts](server/src/lib/shaders/selfWrite.ts)): the shader router calls `noteShaderSelfWrite(path)` right before every atomic write, and the watcher's shader branch checks `isRecentShaderSelfWrite(path)` before calling `recordSave({direction: "incoming", ...})`. Outgoing recordSave happens in the router. Without this, every Bleepforge save would appear twice in the feed (once as outgoing, once as incoming). The cache + SSE still fire on the same event — the gate is purely on the recordSave call.
- **Toast notifications** (client-side, [client/src/lib/shaders/localSaves.ts](client/src/lib/shaders/localSaves.ts)): a per-window TTL map of paths just saved by this client. Wrapped into `shadersApi.save / create / deleteFile` so any code path that writes goes through it. The toast bridge ([shaderToasts.ts](client/src/lib/shaders/shaderToasts.ts)) skips events whose path is in the local-saves map. This is per-WINDOW (not per-server) because the "did *I* initiate this" question is about the user's interaction, not the system's writer — Window A's save echo should be silent in Window A but loud in Window B. Same TTL pattern as the server selfWrite but kept separate because the question is different. Suppression isn't all-or-nothing: the same SSE event still drives the catalog refresh + the cross-window list refresh; only the toast is gated.
...
- **Uniform types**: `bool`, `int`, `float`, `vec2`, `vec3`, `vec4`, `sampler2D`. Each user-declared `sampler2D` gets its own AssetPicker in the uniform controls; the runtime allocates a dedicated WebGL2 texture unit starting at unit 1 (unit 0 stays reserved for the built-in `TEXTURE` / `u_texture`). `mat2/3/4` and the various `usampler*` / `samplerCube*` / `*Array` variants are still out of v1.
- **Hints** — split into two categories that can coexist on one uniform via Godot's comma syntax (`: filter_linear, repeat_enable`). The parser tracks depth so commas inside `hint_range(0, 1)` don't get mis-split. At most one UI hint per uniform (multiple → reject); sampler hints rejected if applied to non-sampler types.\n  - **UI hints** (drive auto-generated control): `hint_range(min, max[, step])`, `source_color` (aliased `hint_color`).\n  - **Sampler hints** (drive runtime texParameteri + routing on sampler2D): `filter_nearest` / `filter_linear` / `filter_nearest_mipmap` / `filter_linear_mipmap` (set MIN/MAG_FILTER + generateMipmap), `repeat_enable` / `repeat_disable` (set WRAP_S/WRAP_T), `hint_screen_texture` (aliases the sampler to texture unit 0 where `u_texture` / the user-picked test image lives, so a shader using Godot's `uniform sampler2D SCREEN_TEXTURE : hint_screen_texture` syntax previews against the picked image without source edits; filter/wrap hints on the same sampler fold into u_texture's params), and `hint_previous_frame` — Bleepforge-specific. A sampler with `hint_previous_frame` reads from a ping-pong back-FBO that holds the LAST frame's rendered output, so the user can author iterative effects (trails, decay echoes, feedback blur) against a stable sampler name. Filter/wrap hints on a prev-frame sampler apply to the back-FBO texture (defaults to LINEAR + CLAMP since trails almost always want smooth sub-pixel sampling). Godot doesn't recognize `hint_previous_frame` — for Godot deployment, comment-toggle it OR set up SubViewport ping-pong manually and bind the same uniform name there.
...
- **Helper functions**: top-level user-defined functions other than `fragment()` (e.g. `vec3 stripes(float spacing) { ... }`) are extracted by the parser and emitted at module scope alongside `main()`. Built-in references in helper bodies (`TIME`, `UV`, `SCREEN_UV`, `MODULATE`, `TEXTURE_PIXEL_SIZE`, `SCREEN_PIXEL_SIZE`, `TEXTURE`, `FRAGCOORD`) are rewritten via [`HELPER_SUBSTITUTION_MAP`](client/src/features/shader/translator/subset.ts) to their underlying uniform / varying / keyword form — so a helper that references `UV` resolves to `v_uv` automatically, without having to declare `uv` as a parameter. `COLOR` is the one exception: it's per-fragment read-write state with no module-scope equivalent in GLSL ES 3.00, so helpers that touch COLOR must take it as `inout vec4`. `void vertex()` / `void light()` are recognized as entry points and **dropped** from the emit — they belong to pipelines the preview can't run.
...
**Compile errors surface in two places**, both driven by a single lifted state in [Edit.tsx](client/src/features/shader/Edit.tsx). The PreviewPane shows a red banner below the canvas with line-anchored entries (`line 12: 'foo' : undeclared identifier`); the CodeMirror editor shows matching gutter markers via [`@codemirror/lint`](https://www.npmjs.com/package/@codemirror/lint)'s `lintGutter()` + imperative `setDiagnostics`. Both translator-level errors (parser rejections) and WebGL compile errors collapse to a shared `ShaderDiagnostic` shape in [diagnostics.ts](client/src/features/shader/diagnostics.ts) so the rendering surfaces don't drift. Diagnostics that can't be line-mapped (a parser error with `line: null` or a WebGL error whose `userLine` is null — typically prelude-level issues we'd never expect a user to see) appear in the banner only. Out-of-range lines (parser reports line N but the user deleted text and the doc is shorter now) are filtered before reaching the editor's `setDiagnostics` — CM6's `doc.line()` throws on invalid input.
...
UI subscribers: every list/edit page wires `useSyncRefresh` for its domain (item, karma, quest, dialog, balloon, npc), the dialog graph view subscribes for the active folder, `ItemIcon` re-fetches its descriptor on item events (so a Godot-side icon change shows up live), and `useCatalog` (autocomplete) bridges through the catalog-bus so it also refreshes on any sync event. The same SSE stream also drives the toast notifications via `useSyncToasts` — see "Toast notifications" above for the click-to-navigate / dedupe / pause-on-hover behavior. **Shader events feed the catalog refresh too** — `Bleepforge:shader` is wired alongside `Bleepforge:sync` in [client/src/main.tsx](client/src/main.tsx) so an externally-added/renamed/removed shader flows into the Ctrl+K AppSearch index without a reload.
...
- ~~**Shaders Phase 4 (multi-texture support)**~~ — done. User-declared `sampler2D` uniforms beyond the built-in `TEXTURE` get per-uniform AssetPickers in the uniform controls; the runtime allocates dedicated texture units (starting at 1, capped by `MAX_COMBINED_TEXTURE_IMAGE_UNITS`) and reconciles on every change via `setSamplerTextures()`. Parser also gained a reserved-uniform-names check that rejects collisions with translator-injected symbols (`u_time`, `u_texture`, `TIME`, `COLOR`, etc.) with a specific message — better than the WebGL "redeclaration" error. Sampler hints (`filter_nearest` / `repeat_*`) are accepted as no-ops so shaders copied from Godot tutorials compile without edits.\n- ~~**Shaders Phase 5 (CodeMirror gutter markers)**~~ — done. `@codemirror/lint`'s `lintGutter()` + imperative `setDiagnostics` push parser + WebGL compile errors into the editor's left gutter. Edit.tsx aggregates both into a unified `ShaderDiagnostic[]` ([diagnostics.ts](client/src/features/shader/diagnostics.ts)) that drives both the editor markers AND the preview pane's banner — single source of truth, no drift.\n- ~~**Shaders Phase 6 (helper-function support)**~~ — done. Parser extracts top-level user functions other than `fragment()` (skips `vertex` + `light` entry-points which belong to other pipelines); emit lays them out at module scope alongside `main()`. Built-in references in helper bodies get a per-scope substitution table — `TIME → u_time`, `UV → v_uv`, `SCREEN_PIXEL_SIZE → (1.0 / u_resolution)`, etc. — so helpers see canvas_item built-ins naturally without parameter plumbing. `COLOR` stays main-scope-only because GLSL ES has no module-scope mutable; helpers that touch it take `inout vec4`. Multi-range source map (one range per helper body + the fragment body) so WebGL compile errors map back to the right line whether they're in `fragment()` or in a helper.\n- ~~**Shaders Phase 7a (sampler hints + screen texture)**~~ — done. Sampler hints (`filter_*` / `repeat_*` / `hint_screen_texture`) now have runtime semantics: `filter_*` drives `TEXTURE_MIN/MAG_FILTER` + `generateMipmap`; `repeat_*` drives `TEXTURE_WRAP_S/T`; `hint_screen_texture` aliases the sampler to texture unit 0 (the picked test image) so post-process shaders authored in Godot's native syntax preview cleanly. Comma-separated hint lists (`: hint_screen_texture, filter_linear`) parse correctly via depth-tracking split. Parser categorizes UI hints (at most one per uniform) vs. sampler hints (multiple OK, only on sampler2D); validates mismatches with specific errors. Hint_screen_texture samplers don't get their own AssetPicker — UniformControls renders a read-only "screen texture · driven by the Test image picker above" note instead.\n- ~~**Shaders Phase 7b (ping-pong framebuffers + hint_previous_frame)**~~ — done. Bleepforge-specific hint `hint_previous_frame` on a sampler2D reads from the previous frame's rendered output via a ping-pong FBO pair. Three-pass draw cycle: pass 1 (passthrough) + pass 2 (user shader) both render to the front FBO; a final blit lifts that FBO to the canvas; the FBOs swap roles for next frame. The user shader's prev-frame samplers route to a reserved high texture unit (`MAX_COMBINED_TEXTURE_IMAGE_UNITS - 1`) where the runtime binds the back FBO's color texture each frame. Filter/wrap hints on a prev-frame sampler apply to the back-FBO texture (default LINEAR + CLAMP — trails almost always want smooth sub-pixel sampling). FBOs allocated lazily + re-allocated on canvas resize (trails state lost on resize, acceptable). New SamplerEntry kind: `prev_frame` alongside `owned` + `screen`. Godot doesn't recognize the hint, so the design trade-off is: portability requires either comment-toggling the hint or setting up SubViewport ping-pong manually in Godot; documented in the translator-subset help entry.\n- **Shaders Phase 8+ (deferred)** — `varying` support if we ever build a vertex pipeline; spatial / particles / sky / fog shader types; `#include` directives (would need a resolver for relative `.gdshaderinc` paths).
