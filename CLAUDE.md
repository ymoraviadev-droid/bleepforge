...
Plus **Shaders surface** — Bleepforge's tenth surface, file-based like Assets: the `.gdshader` text files in the Godot project ARE the source of truth, no JSON cache, no `.tres` round-trip. Five phases shipped — browse, view, edit, save, new (from template), duplicate, delete, cross-system usage tracking, CodeMirror editor with GDShader syntax + gutter diagnostic markers (parser + WebGL errors line-anchored), watcher + SSE for live cross-window sync, a live WebGL2 preview canvas that re-translates GDShader → GLSL ES on every edit with auto-generated uniform controls (sliders for `hint_range`, color pickers for `source_color`), AND multi-texture support — user-declared `sampler2D` uniforms get per-uniform AssetPickers and dedicated WebGL texture units. Covered in the "Shaders surface" section below.
