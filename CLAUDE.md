...
Plus **Shaders surface** — Bleepforge's tenth surface, file-based like Assets: the `.gdshader` text files in the Godot project ARE the source of truth, no JSON cache, no `.tres` round-trip. Phases 1 + 2 are shipped — browse, view, edit, save, new (from template), duplicate, delete, cross-system usage tracking, CodeMirror editor with GDShader syntax highlighting, watcher + SSE for live cross-window sync. Phase 3 (deferred) adds a GDShader → GLSL ES subset translator and a live WebGL2 preview canvas you can point at any project image. Covered in the "Shaders surface" section below.
...
client/src/
  ...
  features/     user-facing pages, one folder per domain — asset, balloon,
                ...
                features/shader/ Phases 1 + 2 ship List + Edit (full
                CodeMirror editor) + ShaderCard + ShaderRow + UsagesPanel
                + CodeEditor + gdshaderLang + NewShaderModal + shaderMenu
                + format.ts. Phase 3 adds a translator/ subfolder +
                PreviewCanvas + UniformControls alongside the editor.
